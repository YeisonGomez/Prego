...
(function() {
  ...
  angular.module('oauth.chaira', ['oauth.utils'])
    .factory('$ngCordovaChaira', chaira);

  function chaira($q, $http, $cordovaOauthUtility) {
    return { signin: oauthChaira };

    /*
     * Sign into the Chaira service
     *
     * @param    string clientId
     * @param    object options
     * @return   promise
     */
    function oauthChaira(clientId, options) {
      var deferred = $q.defer();
      if(window.cordova) {
        if($cordovaOauthUtility.isInAppBrowserInstalled()) {
          var redirect_uri = "http://localhost/callback";
          ...
          var flowUrl = "http://chaira.udla.edu.co/api/v0.1/oauth2/authorize.asmx/auth?client_id=" + clientId + "&redirect_uri=" + redirect_uri + "&response_type=token&state=xyz";
          if(options !== undefined && options.hasOwnProperty("auth_type")) {
            flowUrl += "&auth_type=" + options.auth_type;
          }
          var browserRef = window.cordova.InAppBrowser.open(flowUrl, '_blank', 'location=no,clearsessioncache=yes,clearcache=yes, zoom=no');
          browserRef.addEventListener('loadstart', function(event) {
            if((event.url).indexOf(redirect_uri) === 0) {
  ...
  chaira.$inject = ['$q', '$http', '$cordovaOauthUtility'];
})();
...
(function() {
  ...
  function cordovaOauth(
    ...
    $ngCordovaDribble, $ngCordovaPocket, $ngCordovaMercadolibre, $ngCordovaXing, $ngCordovaNetatmo, $ngCordovaChaira) {
